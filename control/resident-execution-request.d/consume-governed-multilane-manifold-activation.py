...
def consume(source_root: Path, runtime_root: Path, *, runner=subprocess.run) -> dict[str, Any]:
    ...
    for node in nodes:
        ...
        if disposition in REUSE_DISPOSITIONS:
            outcomes.append({"task_id":child_id,"disposition":disposition,"state":"REUSED_NO_EXECUTION","execution_attempted":False,"authority_effect":"NONE"})
            continue
        if disposition in EXTERNAL_DISPOSITIONS:
            external_pending = True
            outcomes.append({"task_id":child_id,"disposition":disposition,"state":"EXTERNAL_OWNER_OBSERVATION_REQUIRED","execution_attempted":False,"competing_claim_created":False,"authority_effect":"NONE_EXTERNAL_OWNER_RETAINED"})
            continue
