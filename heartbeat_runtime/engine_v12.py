...
class HeartbeatRuntime(LegacyWorkerCoordinator):
    ...
    only advances its own carrier state and projects an observational snapshot of
    already-existing worker/task state.

    Worker lifecycle authority remains outside the carrier and is exposed
    separately as ``heartbeat_runtime.worker_runtime.WorkerCoordinator``.
    """
    ...
    def _carrier_worker_observation(self, registry: dict[str, Any], epoch: int) -> dict[str, Any]:
        ...
        return {
            ...
            "state": "OBSERVING" if observations else "IDLE",
