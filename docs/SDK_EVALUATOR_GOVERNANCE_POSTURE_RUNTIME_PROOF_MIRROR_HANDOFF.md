...
The authentic receipt must bind the exact manifest/graph hash when present, projected-input hash when recognized GRG projection is exercised, transition_request_sha256, posture instance id/hash, `resolution_authority=INTERLOCK_INTR`, `posture_bound_execution=true`, `sdk_resolved_posture=false`, and a custody/result locator. No GitHub Actions result substitutes for this receipt.

## Authority boundaries
TV/TVC remains credential authority. Interlock/InTr remains transition/posture authority. The SDK represents/transports/binds evidence only. Heartbeat is observability only. GitHub has no runtime authority.

## Adjacent worker lifecycle
`SDK-TT-PURPOSE-BOUND-WORKER-RUNTIME-PROOF-001` remains a separate downstream consumer. It may reuse this governed SDK-to-runtime seam after it is proven, but this goal does not absorb worker materialization, bounded purpose execution, retirement, or records-only reconstruction.

## First implementation seam
Add only the bounded request-specific consumer and dispatcher registration needed to invoke `run_evaluator_governance_manifest` from the existing resident carrier. Reuse current WorkerCoordinator/ProcessWorkerAdapter/Interlock paths where execution admission is required; do not repurpose the READ_REVIEW evaluator lane and do not create a parallel authority plane.
