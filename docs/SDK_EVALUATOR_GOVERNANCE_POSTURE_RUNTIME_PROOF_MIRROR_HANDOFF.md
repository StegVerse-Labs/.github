...
Canonical generation 70 was re-read before mutation. Master Records still contained no authentic `RESIDENT_REQUEST_DISPATCH_VISIT` transition for `sdk_evaluator_governance_posture`. Tracing the existing carrier identified the concrete source carriage defect: `scripts/dispatch_resident_execution_requests.py` retained the selector visit and machine result in `resident-request-dispatch.latest.json` but did not submit that exact dispatch transition through the existing canonical state-transition custody client.

The existing dispatcher now retains only the `sdk_evaluator_governance_posture` attempted visit through `workers/canonical_state_transition_custody.py`. The required evidence item uses the current canonical six-field Master Records evidence contract and binds the exact selector, consumer ref, attempted=true, task/request identity, machine-result digest, and machine result. This does not change consumer selection, invocation order, request execution, failure handling, or dispatcher authority. The dispatch receipt carries the returned Master Records custody status for observation, but dispatch behavior remains non-authorizing.

This is source repair only. Authentic progression remains prohibited unless Master Records returns `RECORDED + reconstruction_status=PASS + required_evidence_validation_status=PASS` with exact receipt/reconstruction digest equality. No authentic runtime transition is claimed by this repair.
