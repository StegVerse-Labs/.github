...
## 2026-09-21 receiver / Universal InTr port ownership repair

The next deterministic execution collision was loopback ownership: the Universal InTr profiled ingress and machine HIL `combined_gateway` receiver both defaulted to port 8765, and legacy route materialization reused `STEGVERSE_HIL_RECEIVER_PORT` for the InTr ingress. The existing services are now separated without adding a runtime: Universal InTr remains on 8765 under `STEGVERSE_UNIVERSAL_INTR_PORT`; the machine HIL receiver defaults to 8877 under `STEGVERSE_HIL_RECEIVER_PORT`; Gateway continues to learn the actual receiver origin from the retained machine receiver receipt. Legacy HIL route materialization no longer consults the receiver-port namespace.
