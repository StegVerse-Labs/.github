...
PR #1706 reconciled the canonical task record and this handoff with the already merged reusable-lifecycle work and the latest runtime observation. A later `main` advance did not retain those file changes, so this follow-up reapplies the same canonical reconciliation from current `main` without altering runtime predicates or authority boundaries.
...
PR #1706 reconciliation: validated and merged, but later `main` no longer retained those file changes; this branch reapplies them from current `main`.
