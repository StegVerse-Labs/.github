...
This repair materializes only that already-referenced artifact. Its finite expiry candidate is exactly the handoff's existing `runtime_window_beats=4096`. A regression test requires `WorkerCoordinator._expiry_budget()` to resolve `(4096, "TASK_CLASS_COST_BASIS")`. No authentic claim/fence is inferred.
