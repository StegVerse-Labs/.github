...
## Runtime transition-order correction — registry generation 121

Re-reading the executable handoff, WorkerCoordinator source, and StegAgents runtime exposed a canonical projection error: the task record had moved TV/TVC resident credential materialization ahead of the WorkerCoordinator claim/fence transition.

That ordering is incorrect. The existing merged path is:
...
TV/TVC credential materialization is an input to the warrant-policy transition, not a precondition for WorkerCoordinator to mint the fresh claim/fence. No authentic transition for this SDK lineage is currently retained in Master Records, so the next authentic transition remains `WORKERCOORDINATOR_CLAIM_FENCE_BOUND`.
