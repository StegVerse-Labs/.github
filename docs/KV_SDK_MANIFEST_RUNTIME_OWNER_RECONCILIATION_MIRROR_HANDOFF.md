...
## Source-candidate owner reconciliation — 2026-09-24

The canonical Task Registry advanced to generation **222** during this review; the original KV Goal remains absent from active registry entries, while its exact executable handoff and COSV projection persist. This does not establish an admitted successor or permit bypassing AI_SESSION_GATE. The separately existing `STEGOS-DEVICE-KV-SKAP-ROUNDTRIP-001` owns its distinct runtime proof; it must not be repurposed as the original connection-revalidation Goal or a generic custody fixer.

The documentation/coordination candidate is `.github` PR **#2662**, with existing exact-head deterministic repository suite PASS at head `3287fda865a065cb288f28c91a8c98ed6741736b`. This validated the source handoff and invariants only; no owner claim, provider result or Master Records closure was minted.

Under the existing CVK repository, **draft** PR **#222** now contains a scoped proposed-versus-native-accepted custody source repair, nonauthorizing README/handoff updates and adversarial tests. Default `close_lifecycle` now returns a proposed custody record with no terminal completion and refuses caller-authored JSON confirmation at its public entry point. Actual completion requires the existing canonical Master Records native client and separately verified InTr replay via an admitted resident adapter. No new custody store, authority, scheduler, worker or SDK runtime is installed. Treat its native-client test fixtures as `TEST_ONLY_NOT_AUTHENTIC_MASTER_RECORDS`, never native acceptance evidence.

The existing canonical `.github` client in `workers/canonical_state_transition_custody.py` already supplies canonical `build_state_receipt`, `submit_state_receipt` and `reconstruct_state_receipt`. The approved runtime integration must reconcile the independently governed native replay method before binding the candidate's `replay_state_receipt` adapter; **that adapter is not installed by the source PR**. Do not merge draft PR #222 or call this integration complete until the latest-generation owner/collision disposition and authentic AI_SESSION_GATE permit it.

No KV-specific SDK route was installed: the current SDK route table does not publish one, and the existing Universal InTr path already handles generic manifest state-transition transport. A new route requires positive proof that compatibility cannot be achieved through the existing route, plus canonical owner admission. Neither source PR authorizes Google consent, CONNECT/VERIFY, KV #2 materialization or a second device.

## Actual native-client and Universal InTr API reconciliation — 2026-09-24

Reconciliation against current source in `.github/workers/canonical_state_transition_custody.py` confirms the existing native client exports exactly `build_state_receipt`, `submit_state_receipt` and `reconstruct_state_receipt`. The CVK draft originally required a fourth `replay_state_receipt` method that **does not exist** on that client. The existing `.github/workers/manifest_state_transition_intr_ingress.py` instead consumes independently reported `master_records_replay.status=PASS` in its governed WorkerCoordinator result, and the SDK client `stegverse/manifest_state_transition_runtime.py` validates `replay_status=PASS` and canonical manifest/graph/route/every-transition closure. These are separate native owners.
