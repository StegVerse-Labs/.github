...
4. Authentic current-device runtime re-entry — required and pending. It consumes preserved browser storage plus the merged/propagated repair and produces the one device-local retry observation needed by this Goal Task.
...
No provider credential/session operation is required for this local retry. No new user-authentication event is required for this schema-repair completion, and device or node identity must not be treated as such an event.
...
Source construction, CI, merge state, and public-source observation are already satisfied. They do not prove the remaining authentic device-local predicate.
