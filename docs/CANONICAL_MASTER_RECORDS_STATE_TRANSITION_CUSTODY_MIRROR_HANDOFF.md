...
PR #2400 repairs only the missing carriage seam:
- the exact claim/fence Master Records closure is retained on the purpose task in both graph and manifest modes;
- the manifest-bound worker bridge requires that exact `WORKERCOORDINATOR_CLAIM_FENCE_BOUND` closure before building the worker request;
- that exact closure is forwarded as `graph_predecessor_master_records_transition`;
- the existing StegAgents transition recorder therefore binds `TV_TVC_WARRANT_POLICY_VERIFIED` to the exact prior receipt rather than falling back to a bare worker-claim reference.
