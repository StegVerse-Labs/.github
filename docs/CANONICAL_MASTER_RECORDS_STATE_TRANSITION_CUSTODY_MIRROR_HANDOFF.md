...
## Organization receipt resident carriage repair — rebased current main 2026-09-21
...
The first validation attempt exposed a second bounded seam in the same carriage path: the control-plane package validator rejected the hidden org-contract path because it was not exact-allowlisted. The repair exact-allows only `resident-runtime/aggregate_repo_transition.py` and `.stegverse/transition-ledger/org-contract.json`; it does not open a broad `resident-runtime/` or `.stegverse/` prefix. Historical failed run: `35673580314`.

This repair is rebased onto current canonical main after the earlier merge attempt conflicted with concurrent generation advancement. No runtime execution, organization receipt existence, or Master Records completion is claimed by source/CI.
