...
September 2026  
**Co-authored Working Draft — Revision 2**
...
Experiment 2 asks a subsequent question. Once non-emission has been admitted as observable state, does sustained silence remain one continuous state, or can successive bounded periods of silence remain distinct state transitions when the architecture preserves sufficient temporal and predecessor-state resolution?
...
This is a representational difference, not evidence that one architecture is categorically correct or defective. Two architectures may preserve the same externally observable non-emission while retaining different temporal structure.

The experiment supports the following bounded conclusion:

> Once non-emission has become observable machine-readable state, successive periods of non-emission can remain distinct state transitions when an architecture preserves their observation boundaries and predecessor-state relationships. A returned trace that exposes one continuous interval should not be treated as proof that finer internal distinctions do not exist; it establishes only the resolution exposed by that evidence.
...
Experiment 2 shows that representational differences can also occur at the level of temporal and predecessor-state resolution before semantic meaning is resolved, and that those differences can survive controlled governance, custody, replay, and reconstruction.
...
4. comparing exposed temporal/state-transition resolution during sustained non-emission;
