...
Continuation inspection found one concrete addressability seam after successor creation. The generic resident dispatcher already registered `gadi_runtime_observation`, and `refresh_sovereign_worker_runtime_source.py` already carries both `workers/` and `control/resident-execution-request.d/` into the local runtime. However, the canonical one-selector portable bridge `scripts/refresh_and_dispatch_resident_requests.py` did not admit `gadi_runtime_observation` in `ALLOWED_TARGET_CONSUMERS`, so an exact refresh+dispatch invocation for GADI could fail before the registered consumer was visited.

PR `StegVerse-Labs/.github#1608` repairs only that selector gap and adds regression coverage confirming:
...
This repair creates no new resident, scheduler, listener, heartbeat, WorkerCoordinator, claim/fence plane, InTr authority, credential route, hosted fallback, or execution authority. It does not prove that the current iPhone refreshed or visited the request. Authentic runtime evidence remains mandatory.
...
1. Validate and merge PR #1608 if exact-head repository checks pass.
2. Observe the authentic resident local-source refresh and exact dispatch for `gadi_runtime_observation`.
