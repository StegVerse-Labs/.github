...
## 2026-09-24 exact household request priority and ingress follow-up

Current GitHub review verified the canonical task shard still says `PROPOSED` with only `INGRESS_ADMITTED` allowed next, the household resident request is `REQUESTED`, and the existing dispatcher selector `canonical_work_coordination` maps to the canonical bootstrap consumer. The standing Healer schedule remains source-merged; no authentic resident consumption or Master Records closure was accessible through the available GitHub repository evidence. This absence is not proof that a resident invocation failed.

A concrete source-order issue was found: the canonical consumer wrapper unconditionally executed an unrelated MIR subprocess with a 1200-second timeout before visiting any household request, and the legacy request set did not prioritize the explicit goal. The scoped correction on this branch reorders the exact household request first and skips unrelated MIR preemption only when `--goal-task-id ERL-HOUSEHOLD-ECONOMIC-CONDITIONS-SITE-001` is explicitly supplied. Other-goal and unscoped MIR-first routing is retained. Added focused tests for targeted and other-goal routing. Source changes are not presented as proof of the former runtime failure or a successful resident retry.

Next authentic resident cycle must retain the exact household request-consumption receipt at `receipts/sovereign-host/canonical-work-erl-household-economic-conditions-site-request-consumption.latest.json` and the corresponding dispatch and Canonical Work/InTr dispositions. Consume its first authenticated rejection, deferral, exception or admission; fix the owning boundary if needed. On admitted ingress obtain fresh WorkerCoordinator claim/fence, run existing household worker, retain and reconstruct its evidence through Master Records, and advance governed ERL-to-Site publication with served-body verification. Do not treat hosted source tests, candidate observations or absent repository receipts as runtime authorization. No BEA-derived operation without authentic TV/TVC `decision=READY`; preserve `UNKNOWN` otherwise. Target independently supported public release by September 30, 2026, without unsupported household welfare conclusions or a device/observation prerequisite.
