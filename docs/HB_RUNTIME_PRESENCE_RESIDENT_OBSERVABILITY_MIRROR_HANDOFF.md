...
Consumer repository: `Admissible-Existence/GCAT-BCAT`.
...
The wrapper invokes only the canonical shared
`heartbeat_runtime/runtime_presence_projection.py` projector. Bootstrap, native
installation, and source refresh carry the profile and wrapper into the resident
runtime without copying mutable receipts.

Local evidence slots remain distinct: Ecosystem Chat resident request consumption,
resident targeted execution, and independent parent activation. TVC route admission,
LLM-adapter measured execution, Master Records same-execution reconstruction, Site
readiness, and the four Decision Envelope destination projections remain external owner
predicates.
