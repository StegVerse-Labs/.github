...
- `custodian` remains the canonical neutral role term for the party holding/source-controlling historical records; `historical custodian` and `personal-record custodian` are established StegVerse usages, while `evidence holder` has no distinct canonical StegVerse meaning;
