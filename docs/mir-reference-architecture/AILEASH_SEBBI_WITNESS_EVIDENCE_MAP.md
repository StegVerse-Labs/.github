...
| sebbi.pro positively attested the exact flavorflow tip in one unauthenticated call | `USER_RETAINED_PUBLIC_RESPONSE` | User queried `/x/witness/attest?peer=flavorflowstrategy.uk&tip=27846a6a94b2ef76687a22d1a51fbc6a6a302fc691e8e1a361dc16b49a576653`. Response returned `witnessed=true`, first/last observation times, `times_observed=253`, sealed chain hash and `block_index=560`. | This reproduces the core Appendix A R4 public-read behavior for one exact peer/tip. It does not prove peer identity, current liveness, historical completeness, or control-domain independence. |
...
| `self-consistent` is intentionally weaker than third-party verification | `INDEPENDENTLY_OBSERVED_PUBLIC_RESPONSE` + `SOURCE_INSPECTED` | Public response legend and `modules/witness.py` state both halves of the check came from the submitter and the term is not third-party verification. | Directly supports the epistemic-boundary behavior described in Appendix A R4. |
| historical sebbi.pro witness blocks carry vocabulary values without the corresponding vocabulary definitions | `SOURCE_INSPECTED` + `USER_RETAINED_COUNTERPART_COMMUNICATION` | `justrightdecorators-ops/aileash@f824ac83c5843053bdeca7eba53b77afa99465f5:modules/witness.py` states v1.1 and earlier sealed `confirmed`, v1.2 renamed the same check to `self-consistent`, and old blocks cannot be altered. The seal payload contains `witness_version` and the term values but not their vocabulary definitions. Current responses attach `*_vocabulary` after lookup, and the source explicitly says sealed blocks travel while legends do not. | Justin's specific historical artifact-portability gap is source-inspected. Do not promote this into blanket R10 nonconformance for all sebbi.pro commitments until the R6 scheme/vocabulary publication coupling and historical preservation are inspected. |
...
| exact raw OTS artifact for stamp `1789455657` was retained | `USER_RETAINED_PUBLIC_RESPONSE` | User retrieved `/x/ots/proof?ts=1789455657`, returning digest, 721 proof bytes, matching proof SHA-256, raw `ots_base64`, `state=pending`, no Bitcoin block heights, and pending calendars. | The exact proof artifact exists and is retained, but it is explicitly pending and therefore is not Bitcoin-confirmed evidence. |
| retained OTS base64 matches the returned proof length/hash | `INDEPENDENT_LOCAL_RECONSTRUCTION` | Decoding the supplied `ots_base64` produced exactly 721 bytes; SHA-256 exactly matched the returned proof hash. | The retained raw proof bytes are internally consistent with the service response. This does not convert a pending OTS proof into Bitcoin confirmation. |
| Bitcoin anchoring of stamp `1789455657` is independently verified | `NOT_ESTABLISHED` | The retained proof is `state=pending`, has no Bitcoin block height, and contains pending calendar attestations. | This proof cannot satisfy a Bitcoin-confirmed external-time claim yet. |
| AILeash publishes an unauthenticated single-tip witness-attestation route | `SOURCE_INSPECTED` + `USER_RETAINED_PUBLIC_RESPONSE` | Public source describes `GET /x/witness/attest?peer=&tip=`; the user successfully exercised it for the exact flavorflow tip. | Route existence, unauthenticated use, and exact positive behavior are established for this peer/tip. |
...
| Justin Dobson's star-versus-graph and collusion/simultaneous-failure analysis is an implementation-informed contribution | `COUNTERPART_REPORTED` with source-text provenance | Evidence Custody Seam credits Justin in place. | Preserve the attribution and architecture insight; do not infer network topology facts not directly evidenced. |
| A witness count alone proves independence | `NOT_ESTABLISHED` / rejected by current seam text | Current seam text replaces witness count with an independence/control/failure-domain figure. | Node/member count must not be promoted to an independence claim. |
| AILeash witness topology currently satisfies the collusion/failure-domain metric at a particular numeric value | `NOT_ESTABLISHED` | Public peer list plus one exact positive peer/tip attestation show a concrete witness relationship, but do not establish ownership/control/failure-domain independence among all peers. | No numeric resilience figure is assigned by StegVerse from these artifacts alone. |
...
## Joint-review v0.5 follow-up

A user-retained LinkedIn DM records Justin Dobson accepting the v0.5 merge decisions and R10 separation, proposing a base-version-only rule for reviews sent outside the shared document, requesting a sebbi.pro R10-fail row, requesting a clean-room rerun with the submitter removed, and asking for a naming shortlist.

Source inspection now independently substantiates the narrow historical witness-artifact portability gap behind Justin's R10 comment. It does not yet establish blanket R10 failure across all commitment types because the R6 scheme/vocabulary publication coupling has not been completely audited.

The requested clean-room rerun remains pending and does not alter the current `NOT_ESTABLISHED` classification for StegVerse independent reproduction of the 784-record / 30-commitment run.
...
The supplied proof for stamp `1789455657` is useful but intentionally non-promoting for Bitcoin. The raw artifact is internally consistent with the service response. Its own state is `pending`; it carries no Bitcoin block height and names pending OpenTimestamps calendars. It therefore proves only that a concrete OTS proof artifact was retained and that the artifact is pending, not that the digest is in Bitcoin.

## Relation to R4 topology

The public peer list plus the exact positive flavorflow witness response demonstrate at least one concrete cross-system witnessing relationship. They still do not establish the administrative, ownership, infrastructure, or correlated-failure independence required by R4. The map therefore does not convert visible peers or observed relationships into a resilience score.
...
- Source inspection may establish published implementation behavior or historical-format facts, but it does not establish authentic runtime execution.
