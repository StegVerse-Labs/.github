...
### 2026-09-24: Stop stale-branch proliferation

Conflicting census PRs #2574 and #2617 are superseded by a single fresh-main successor. Historical bulk baselines remain 3,563 inspected, 1,006 *structural* candidates across `.github`, Site and private StegCore; exact run IDs and artifact digests are in `control/repository-hygiene-ecosystem-census-20260921.json`. Current `.github` repository setting `delete_branch_on_merge=false` has been independently observed; leaving merged PR heads in place is avoidable branch debt, but closing a PR without merging is never itself branch-deletion authority. The existing `HYGIENE-BRANCH-REF-RETIREMENT` repository-administration owner must consume recorded merged-head or independently signed explicit release evidence, verify no active PR/task/release/evidence retention and exact live ref SHA, then retire that ref with atomic `--force-with-lease` semantics. GitHub Actions retains validation/evidence-only permissions; no classification job may mutate refs and no new credential path is introduced. A guarded native-administrator bulk retirement utility and its local adversarial tests are installed separately from the read-only census. Actual deletions remain unproven until verified repository-authority receipts exist.


### Native ref-retirement implementation and live disposition — 2026-09-24

`.github` PR #2651 merged native retirement source and its read-only CI test workflow (`a0ecb920f70d5e0f2a35df68a13c6c8bf05c79dd`). Hosted run `36041822725` completed SUCCESS; eight adversarial source tests passed. The utility requires live exact tip and origin checks, rejects protected refs and open PR heads, checks merged PR-head identity or independent release and uses expected-tip force-with-lease plus ref-absence readback; CI has no mutation authority. Test mocks demonstrate source behavior, not actual deletion.

Authenticated current-state inventory of the prior approved queue finds all 32 TVC refs still present, unprotected and without an open PR head. Exact Git ref reads also confirm two superseded closed-unmerged `.github` PR heads and the merged #2651 implementation head still exist. Total disposition is **0 actually deleted / 35 retained**, not a completed retirement. Record: `control/repository-hygiene-native-ref-retirement-disposition-20260924.json`; native admin intake: `control/repository-hygiene-ref-retirement-admin-intake-20260924.json`.

The GitHub connector exposes read/ref update but no branch deletion or repository setting mutation. `.github` still has `delete_branch_on_merge=false`. Critically, packet field shapes and digest syntax are not authentic Interlock/InTr/organization/Master Records proof; the current queue lacks per-ref authenticated release and custody readback. The existing authority owner `HYGIENE-BRANCH-REF-RETIREMENT` owns that missing evidence and any native deletion under existing TV/TVC credentials. No new runtime, connected device or GitHub Actions write permission is authorized. Preserve all 35 refs until exact signed/authoritative release is authenticated and native delete/readback returns evidence; actual deletion and post-deletion org/Master Records closure must be recorded separately. Bounded validation claim can be released with this explicit blocker and owner rather than left dangling.
