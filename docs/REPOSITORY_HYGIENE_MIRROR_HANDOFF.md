...
### Ecosystem census reconciliation — 2026-09-23

The historical Sept 22 first-baseline evidence is preserved by `control/repository-hygiene-ecosystem-census-20260921.json` under existing `HYGIENE-CAUSAL-ROOTS-001`, COSV `10100000100000`, against the currently observed canonical Task Registry generation 211. The first authenticated heavy baselines cover `.github` 2,324 inspected / 726 structural candidates (run 35688555518, artifact 10678215761), Site 1,100 / 250 (same run, artifact 10678315580), and private StegCore 139 / 30 (in-repository run 35688973007, artifact 10677971716): 3,563 inspected / 1,006 structural candidates, not owner-approved or deletion-authorized. Site's central read-only census does not constitute an installed Site caller. The central matrix now includes only public `.github` and Site; private StegCore retains its independently validated in-repository caller. These are pinned historical heads, not assertions about live Sept 23 branch counts. The pre-existing PR #2574 conflicted with subsequent unrelated mainline changes and is superseded by a main-based replacement rather than overwriting newer canonical work. Organization-level owner-release proofs are the next evidence gate; an authenticated census alone routes zero new refs.
