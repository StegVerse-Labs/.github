...
Updated: 2026-08-27T17:15:00-05:00
...
`.github` remains `VECTOR_REQUIRED`. Ten active vectors are indexed today, but the global worker registry and organization task registry contain additional active machine tasks without complete canonical vector projection. No digits may be invented to close that gap.
...
- 28 active worker task IDs lack canonical COSV coverage.
- 14 organization-registry task IDs lack canonical COSV coverage.
- Total active .github task IDs lacking canonical COSV coverage: 42.
