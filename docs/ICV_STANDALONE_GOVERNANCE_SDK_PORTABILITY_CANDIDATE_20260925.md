...
StegCore deliberately uses canonical source name `stegcore` v0.3.0 and an isolated release-build rename to public distribution `stegverse-stegcore` (same import namespace). See its `docs/STEGVERSE_STEGCORE_PYPI_DISTRIBUTION.md`, publishing workflow and existing propagation issue `StegVerse-Labs/StegCore#203`. Published v0.3.0 and anonymous installation were not established by that owner at the last recorded check; verify live release tag, trusted-publishing job, public wheel/sdist digests and an anonymous offline-wheel install before issuing customer instructions.
