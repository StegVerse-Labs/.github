...
Relevant merged evidence includes `.github` #1461, #1465, #1487, #1522, #1533 and TVC #386/#387. #1533 strengthened portable dispatch so a complete StegBrowser refresh+dispatch requires the dedicated consumption receipt to be present, identity-valid, hash-bound, and tied to the pinned TVC SHA.

## HANDOFF_READY fail-closed repair

Post-#1533 inspection found a semantic gap in the evidence acceptance rule.

The StegBrowser consumer emits:
...
when another task owns `tvc-handoff/private-source-request.json`. In that case the pinned StegBrowser TVC request is **not staged**.

However `scripts/refresh_and_dispatch_resident_requests.py` still included `HANDOFF_READY` in `STEG_BROWSER_TVC_ALLOWED_OUTCOMES`, so a dispatcher-level success plus that dedicated receipt could satisfy `target_consumption_valid` and incorrectly produce `REFRESH_AND_DISPATCH_COMPLETE`.

The current repair narrows successful StegBrowser target-consumption outcomes to:
...
`HANDOFF_READY` remains a legitimate consumer observation but now fails closed at the portable bridge as `REFRESH_COMPLETE_DISPATCH_INCOMPLETE` until the exact StegBrowser request actually occupies the private-source slot.

A deterministic regression test covers the occupied-slot case and requires:

```text
target_consumption_receipt_observed = true
exact_target_consumption_evidence_observed = false
state = REFRESH_COMPLETE_DISPATCH_INCOMPLETE
```

This repair changes no task identity, selector, scheduler, dispatcher, heartbeat, credential path, transition authority, WorkerCoordinator authority, machine requirement, or downstream TVC promotion wiring.
...
1. Validate and merge the `HANDOFF_READY` fail-closed repair.
2. When an authorized sovereign resident or admitted Remote Computer ephemeral Node is reachable, verify its local `.github` source contains the merged fail-closed repair or a later compatible main.
3. Execute the existing refresh+dispatch bridge for exactly `stegbrowser_tvc_source_promotion`; only an outcome proving the exact request is staged may produce a complete bridge receipt.
