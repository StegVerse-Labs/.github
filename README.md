...
A newly spawned self-heal WorkerCoordinator is treated differently from a previously proven worker that later becomes stale. If the new process remains alive but synchronous local source refresh, rendezvous handling, or resident-request dispatch delays its first task-capable tick beyond the short supervision proof window, supervision retains that exact PID in a pending-first-tick state instead of terminating or duplicating it. The pending process is not promoted to runtime presence until its `runtime_tick` advances beyond the recorded startup baseline; once that happens, normal freshness and task-capability semantics apply. A process that actually exits still follows the existing fail-closed repair path.
