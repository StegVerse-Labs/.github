...
### SDK TT WorkerCoordinator preclaim ordering reconciliation

The preserved SDK TT execution lineage now reconciles to canonical commit `f883d36adb356e44dace09f07109af351aab29a6`, which fixes the WorkerCoordinator preclaim bug where `manifest_runtime_request_present` was consumed before initialization. A regression guard now requires initialization before assignment-record use and before `WORKERCOORDINATOR_CLAIM_FENCE_BOUND` custody. No authentic claim/fence receipt is inferred from this source repair.
