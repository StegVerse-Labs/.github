...
The existing targeted resident refresh+dispatch bridge may also address the already-registered SV002 standing-awareness consumers `astra_class_resilience_awareness` and `quantum_resilience_awareness`. Each invocation still selects exactly one consumer through the same generic resident dispatcher. This closes prerequisite addressability for the existing Astra -> quantum -> SV002 activation sequence without introducing batching, a second dispatcher, a scheduler, or any new authority source. Source/bridge availability does not prove either awareness predicate or SV002 execution.
