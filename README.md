...
A self-healed WorkerCoordinator is not terminated merely because its first task-capable cycle takes longer than the short supervision proof window. The existing worker may synchronously refresh already-local source or drain already-materialized resident requests before its first `WorkerCoordinator.cycle()`, and those bounded operations can legitimately outlive the initial presence timeout. When the worker PID is still alive, supervision retains that exact process with `worker_task_capable_cycle_observed=false`, records the startup tick baseline, and rechecks the same PID on the next carrier supervision visit. Presence remains false until the runtime tick advances beyond that baseline. This prevents the HB supervisor from killing a healthy worker mid-dispatch or spawning a duplicate WorkerCoordinator while preserving fail-closed proof of task capability.

A worker that actually exits before the task-capable tick still fails repair and may be restarted through the same canonical self-heal path. HeartBeat supplies only the supervision/reference signal; it does not authorize the retained process, dispatch, claim/fence, credential use, InTr transition, or task execution.
