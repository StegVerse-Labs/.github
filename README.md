...
Generation-161 re-observation for `CONVERSATION-EVIDENCE-INGESTION-CUSTODY-001` confirms the first missing authentic transition remains `RESIDENT_REQUEST_DISPATCH_VISIT`: the retained worker state is still historical and the runtime-presence, resident-dispatch, machine-continuation, and Healer scheduler receipts are absent, while the Healer registry remains `HANDOFF_READY` with `claim_id=null`. No downstream runtime/custody state is promoted.
