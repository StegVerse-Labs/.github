...
## Task session duplication and collision STATUS

Existing canonical Task Registry check-in and hash-linked session-return paths support an additive `session_status_projection` for compact task blocks: `DUPLICATE: UNVERIFIED | CONFIRMED` and `COLLISION: UNVERIFIED | CONJOIN_REQUIRED | REVIEW_REQUIRED | NO_REGISTERED_COLLISION_OBSERVED`. A current checked-out task with an overlapping exclusive component or user-action surface requires owner reconciliation; mere repository overlap or shareable surfaces do not automatically merge tasks. Another retained active check-in for the same goal and mutation scope is a **candidate** only while origin attestation is unavailable. An unobserved external chat session can never be inferred absent from a source-only registry result.

The existing machine `disposition`, generation fence, event predecessor chain and `RETURNED` footer gate remain unchanged. Session STATUS values do not confer execution, checkout or transition permission. Native coordination owner: `TASK-REGISTRY-SESSION-RETURN-ORCHESTRATION-001`; see `docs/TASK_REGISTRY_SESSION_RETURN_ORCHESTRATION_MIRROR_HANDOFF.md`. Exact-head CI and authentic host-origin integration are separate proof gates.
