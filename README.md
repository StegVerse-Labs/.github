...
[Current task census and build plan](docs/ECOSYSTEM_CHAT_TASK_CENSUS_AND_BUILD_PLAN.md) reconciles Registry generation 243, all 172 canonical shards, native Site/Adapter task files and existing provider/sandbox PRs. It distinguishes unfinished runtime/integration from completed reusable source. Existing Adapter #351 has a local, unpublished repair for provider alias/wire-hash versus Chat request-envelope binding, with 115 offline tests and mutation-safety validation passing. Actual Chat-to-ephemeral-provider routing, provider runtime results, browser teardown, original organization/Master Records evidence and release propagation are not established. The report records publication approval required after automatic review rejected the push; it creates no task-state transition or replacement runtime.
