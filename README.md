...
### Host-interchangeable organization custody verification

`ORGANIZATION-BATCH-CUSTODY-REPLAY-001` requires **no external always-on resident or connected device**. Any eligible authorized host can execute source validation and the optional read-only original-ledger selector; actual custody reconstruction follows the original immutable authenticated organization HEAD, complete predecessor-linked receipts, source/evidence bytes and batches through the existing approved return/recovery path. A device inventory result never determines StegVerse task or functional outcome. Unavailable original bytes/authorized transfer remain `UNKNOWN_NOT_AUTHENTICALLY_OBSERVED`, not a worker failure, synthetic reconstructed HEAD or reason to require another device. Authentic session-origin TV/TVC authority, applicable InTr governance and independent Master Records proof remain separate acceptance predicates. See [the canonical handoff](docs/ORGANIZATION_BATCH_CUSTODY_REPLAY_MIRROR_HANDOFF.md#host-interchangeable-execution-invariant--2026-09-25); PR #2739 is a portable optional selector, not a persistent-host deployment.
