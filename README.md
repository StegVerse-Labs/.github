...
The registered continuation task `STEGVERSE001-EVIDENCE-CHAIN-CONTINUATION-001` is independently selectable by the existing WorkerCoordinator after the canonical SV001 execution is already terminal. This prevents downstream evidence observation from being coupled only to retries of the parent SV001 consumer.

The independent `.github` binding is now deliberately an **observer/retry classifier for the custody portion of the chain, not a custody mutation producer**. It never reruns terminal SV001 and it may not invoke the legacy `master-records/orchestration/scripts/watch_stegverse001_autonomy_receipt.py` path to create new custody. The canonical mutation producer is the already-released current-iPhone Site path: exact retained/recovered G23 -> `StegOSWebBootstrap.executeMasterRecordsSv001Custody()` -> fresh root `MasterRecords:SV001Custody` Interlock/InTr decision -> retained exact `ALLOW` -> canonical Master Records custody/reconstruction.

A WorkerCoordinator claim/fence admits only the bounded continuation observer. It does not authorize custody. Before the observer may accept Master Records custody/reconstruction as valid downstream evidence, it requires a retained contemporaneous admission with schema `stegverse.master-records.sv001-custody-intr-admission/v1`, `state=INGRESS_ADMITTED`, `governance_decision=ALLOW`, exact transition `SV001_MASTER_RECORDS_CUSTODY_AND_RECONSTRUCTION`, exact canonical G23 source binding, and `current_governance_decision_observed=true`. A prior SV001 receipt, successful G23 recovery, WorkerCoordinator selection, HB32 reference, legacy watcher output, source merge, or CI result cannot substitute for that current governance decision.

If the current admission or governed consequence evidence is not yet present, the worker returns `HANDOFF_READY` for later machine retry. Malformed, denied, stale, or source-mismatched admission evidence fails closed. The continuation reports `COMPLETED` only after the retained governed chain and SV002 observation predicates themselves return `PASS`. TV/TVC remains credential authority; Master Records remains custody/reconstruction consequence owner; Site remains an execution/proposal surface with no independent authority.
