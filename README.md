...
## Ecosystem Chat task census and integration repair (2026-09-25)

[Current task census and build plan](docs/ECOSYSTEM_CHAT_TASK_CENSUS_AND_BUILD_PLAN.md) reviews Registry generation 243, unchanged native task states, 74 open issue candidates including Local Claude Code #2721, existing provider drafts, and merged optional Grok #352. Approved source snapshots are published in central #2723 and Adapter #351. The provider-envelope repair preserves the original Chat request hash and alias; authentic runtime remains unverified. Seven reusable scenario definitions and `scripts/compose_ecosystem_chat_task.py` extend the existing constructor with hash-bound, source-only single/parallel/fallback plans, conditional adapter selection, explicit browser/API/tool modes and terminal cleanup. These definitions await existing-owner executable binding; no runtime runner is claimed. Local ChatGPT must be independently proven first, Claude Code second; both still require canonical registration, not guessed COSVs.
