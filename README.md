## Task-native worker resource-cost linkage — central preregistration candidate (2026-09-24)

Draft central coordination-only preregistration for `WORKER-TASK-RESOURCE-COST-LINKAGE-001`, [issue #2619](https://github.com/StegVerse-Labs/.github/issues/2619). The current source candidate does not imply authentic AI_SESSION_GATE admission, an assigned COSV, installed SDK linkage, WorkerCoordinator authority or runtime custody. The scoped implementation must be exercised through the **existing SDK manifest-driven purpose_bound_worker processor after its owner admits wiring**; source-only tests cannot be promoted to governed runtime proof. Existing organizational receipt-cost measurement remains under `ORGANIZATION-BATCH-CUSTODY-REPLAY-001`. See `docs/WORKER_TASK_RESOURCE_COST_LINKAGE_MIRROR_HANDOFF.md`.
