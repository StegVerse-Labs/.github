...
The existing SV002 reusable TVC runner is also being corrected to traverse the released same-service installer `--activate` leg instead of bypassing it with direct dispatcher activation; this preserves the existing Healer/reusable-task carrier while making PR #445 correlation evidence reachable. Source change alone does not promote runtime predicates.
