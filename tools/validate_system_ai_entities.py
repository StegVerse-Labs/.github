...
VALID_STATES = {"DECLARED","FEDERATION_REGISTERED","HEARTBEAT_PRESENT","INFERENCE_PROVEN","SYSTEM_AI_ACTIVE","DEGRADED","RETIRED"}
...
def main() -> int:
    ...
    hb = load(HB_REGISTRY)
    if registry.get("schema") != "stegverse.system-ai-entity-registry/v0.2":
    ...
    participants = {row["repository"]: row for row in hb.get("required_participants", []) if isinstance(row, dict) and isinstance(row.get("repository"), str)}
    ...
    for entity in entities:
        ...
        seen.add(entity_id)
        if entity.get("schema") != "stegverse.system-ai-entity/v0.2":
        ...
        if state == "INFERENCE_PROVEN":
            if activation.get("heartbeat_presence_proven") is not True or activation.get("inference_execution_proven") is not True:
                return fail(f"{entity_id}: INFERENCE_PROVEN requires heartbeat and inference execution proof")
        if state == "SYSTEM_AI_ACTIVE":
            required = (
                "federation_membership_established",
                "heartbeat_presence_proven",
                "inference_execution_proven",
                "interaction_boundary_standing_proven",
