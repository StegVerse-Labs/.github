...
class ResidentWorkerPresenceSelfHealTests(unittest.TestCase):
    ...
    def test_alive_worker_with_slow_first_tick_is_retained_for_next_supervision_pass(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            (root / "scripts").mkdir(parents=True)
            (root / "scripts" / "run_worker_runtime.py").write_text("# present\n", encoding="utf-8")
            ...
            with mock.patch.dict(os.environ, {}, clear=True), \
                 ...
                 mock.patch.object(repair.os, "kill") as kill:
                result = repair.ensure_worker_presence(root, carrier_pid=111)
    ...
    def test_pending_worker_becomes_present_only_after_tick_advances_beyond_startup_baseline(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            (root / "scripts").mkdir(parents=True)
            (root / "scripts" / "run_worker_runtime.py").write_text("# present\n", encoding="utf-8")
