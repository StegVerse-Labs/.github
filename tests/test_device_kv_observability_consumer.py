...
CONSUMER = ROOT / "control" / "runtime-observability-consumers" / "device-kv-intr-observation-001.json"
CONTRACT = ROOT / "management" / "HB_RUNTIME_PRESENCE_RESIDENT_OBSERVABILITY_CONTRACT.json"


def test_device_kv_shared_observability_binding_is_non_authorizing():
    ...
    assert consumer["predicate_map"]["admitted_predecessor_observed"]["observed"] is False
    assert consumer["predicate_map"]["device_kv_intr_observed"]["observed"] is False
    ...
    assert consumer["intr_governs_admissible_movement"] is True
