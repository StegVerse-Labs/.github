...
class SovereignStegFinPostBootstrapTests(unittest.TestCase):
    ...
    def test_hosted_environment_fails_closed_before_installer(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = post_bootstrap.activate(root, proof_path=proof, node_marker=node, executor_activation_receipt=executor_receipt, integration_receipt=integration, env={"GITHUB_ACTIONS": "true"}, runner=runner)
    ...
    def test_incomplete_sovereign_proof_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.node(node)
            runner = FakeInstaller(executor_receipt)
            result = post_bootstrap.activate(root, proof_path=proof, node_marker=node, executor_activation_receipt=executor_receipt, integration_receipt=integration, env={}, runner=runner)
            self.assertEqual(result["state"], "FAIL_CLOSED")
    ...
    def test_success_activates_only_released_service_with_clean_environment(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.node(node)
            runner = FakeInstaller(executor_receipt)
            result = post_bootstrap.activate(root, proof_path=proof, node_marker=node, executor_activation_receipt=executor_receipt, integration_receipt=integration, env={"HOME": str(base), "PATH": "/usr/bin", "GITHUB_TOKEN": "forbidden", "TVC_TOKEN": "not-required", "WALLET_KEY": "forbidden", "PROVIDER_SECRET": "forbidden"}, runner=runner)
