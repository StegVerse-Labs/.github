...
class Live009ResidentRunnerTests(unittest.TestCase):
    ...
    def make_valid_runtime(self, root: Path, *, fence: int = 22, terminal: bool = True, terminal_claim: str | None = "MATCH", nested: bool = False, nested_claim: str | None = "MATCH"):
        ...
        assignment = {"task_id": mod.TASK_ID, "claim_id": claim_id, "fencing_token": fence, "source_admission_ref": "handoffs/HEARTBEAT-INDEPENDENT-OSCILLATOR-LIVE-009.json#admission", "source_carrier_event_ref": None}
        (root / "events/master-records-worker-assignment.jsonl").write_text(json.dumps(assignment) + "\n")
        worker_event = {"task_id": mod.TASK_ID}
        if terminal_claim == "MATCH": worker_event["claim_id"] = claim_id
        elif terminal_claim is not None: worker_event["claim_id"] = terminal_claim
        if terminal:
            if nested:
                response = {"transition_id": mod.TERMINAL}
                if nested_claim == "MATCH": response["claim_id"] = claim_id
                elif nested_claim is not None: response["claim_id"] = nested_claim
                worker_event["response"] = response
            else:
    ...
    def test_accepts_nested_terminal_bound_to_same_claim(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td); self.make_valid_runtime(root, nested=True); mod.require_runtime_evidence(root)

    def test_rejects_stale_fence(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td); self.make_valid_runtime(root, fence=21)
            with self.assertRaisesRegex(RuntimeError, "fresh independently admitted"): mod.require_runtime_evidence(root)

    def test_rejects_carrier_derived_assignment(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td); self.make_valid_runtime(root)
            p = root / "events/master-records-worker-assignment.jsonl"; row = json.loads(p.read_text()); row["source_carrier_event_ref"] = "events/heartbeat-runtime.jsonl#packet_id=x"; p.write_text(json.dumps(row) + "\n")
            with self.assertRaisesRegex(RuntimeError, "fresh independently admitted"): mod.require_runtime_evidence(root)

    def test_rejects_missing_terminal_worker_evidence(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td); self.make_valid_runtime(root, terminal=False)
            with self.assertRaisesRegex(RuntimeError, "terminal LIVE-009"): mod.require_runtime_evidence(root)

    def test_rejects_terminal_event_without_claim_binding(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td); self.make_valid_runtime(root, terminal_claim=None)
            with self.assertRaisesRegex(RuntimeError, "terminal LIVE-009"): mod.require_runtime_evidence(root)

    def test_rejects_terminal_event_bound_to_different_claim(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td); self.make_valid_runtime(root, terminal_claim="SHWP-OTHER-G999")
            with self.assertRaisesRegex(RuntimeError, "terminal LIVE-009"): mod.require_runtime_evidence(root)

    def test_rejects_nested_terminal_without_claim_binding(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td); self.make_valid_runtime(root, nested=True, nested_claim=None)
            with self.assertRaisesRegex(RuntimeError, "terminal LIVE-009"): mod.require_runtime_evidence(root)

    def test_rejects_nested_terminal_bound_to_different_claim(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td); self.make_valid_runtime(root, nested=True, nested_claim="SHWP-OTHER-G999")
            with self.assertRaisesRegex(RuntimeError, "terminal LIVE-009"): mod.require_runtime_evidence(root)
