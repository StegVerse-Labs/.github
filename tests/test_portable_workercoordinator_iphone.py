...
def test_terminal_package_is_globally_non_checkoutable():
    ...
    assert admission["claim_state"]=="TERMINAL_NO_FURTHER_CLAIM"
    assert admission["fresh_fence_required"] is False
...
def test_terminal_package_binds_first_canonical_g23_and_retains_duplicates():
    ...
    assert {d["cycle_receipt_sha256"] for d in duplicates}=={
    ...
    assert all(d["custody_eligible"] is False for d in duplicates)
    reset=next(d for d in duplicates if d["cycle_receipt_sha256"].endswith("648d00d4"))
    assert reset["node_id"]=="stegnode-web-2d6daa94e496d451d16bd5619bd30a25"
    assert reset["claim_id"]==terminal["canonical_claim_id"]
    assert reset["reason"]=="RESET_LINEAGE_REEXECUTION_AFTER_CANONICAL_TERMINAL"
...
def test_historical_first_portable_sv001_claim_is_generation_23():
    ...
    assert pkg["terminal_execution"]["canonical_fencing_token"]==generation
