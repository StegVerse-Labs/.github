...
class GlobalRuntimeNodeProfileConvergenceTests(unittest.TestCase):
    ...
    def test_stegbrowser_is_the_retained_node_lifecycle_reference(self):
        ...
        self.assertEqual("REGISTERED_RESIDENT_SELECTOR", row["execution_binding"]["type"])
        self.assertEqual(["stegbrowser_tvc_source_promotion"], row["execution_binding"]["selectors"])
        self.assertEqual("receipts/sovereign-host/canonical-work-stegbrowser-ephemeral-runtime-binding-request-consumption.latest.json", row["execution_binding"]["prerequisite_receipt"])
        self.assertEqual("AUTHENTIC_TVC_SOURCE_PROMOTION_CONSUMPTION", row["resume_stage"])
