...
def test_readme_documents_material_authority_semantics_change():
    readme = (ROOT / "README.md").read_text()
    assert "### Current-iOS interaction authority classification" in readme
    assert "SV001 Master Records custody/reconstruction" in readme
    assert "does not require a human interaction slot" in readme
    assert "still requires current Interlock/InTr governance" in readme
