...
class EcosystemChatInferenceBindingTests(unittest.TestCase):
    def test_independent_parent_is_non_authorizing_and_fresh_fenced(self) -> None:
        ...
        request=json.loads((ROOT/"control"/"resident-execution-request.d"/"ecosystem-chat-parent-001.json").read_text())
        ...
        self.assertEqual(handoff["activation"]["minimum_fencing_token_exclusive"],24)
        self.assertEqual(auth["minimum_fencing_token_exclusive"],24)
        self.assertEqual(fragment["tasks"][0]["admission"]["minimum_fencing_token_exclusive"],24)
        self.assertEqual(request["fresh_fence_minimum_exclusive"],24)
        ...
        self.assertIn("MINIMUM_FENCE_EXCLUSIVE = 24",source)
