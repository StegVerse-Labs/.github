...
class GlobalRuntimeEvidenceConvergenceExecutionTests(unittest.TestCase):
    ...
    def test_global_visitor_reuses_existing_dispatcher_and_bounded_wrappers(self):
        ...
        self.assertIn("stegbrowser_tvc_source_promotion", selected)
        self.assertEqual(("stegbrowser_tvc_source_promotion",), module.TASK_SELECTORS["STEG-BROWSER-EPHEMERAL-RUNTIME-BINDING-001"])
        self.assertNotIn("STEG-BROWSER-EPHEMERAL-RUNTIME-BINDING-001", module.CANONICAL_WORK_ONLY)
    ...
    def test_runtime_profile_map_and_stegbrowser_bootstrap_trigger_global_visitor(self):
        ...
        self.assertIn('STEGBROWSER_RUNTIME_CONSUMPTION_TASK_ID = "STEG-BROWSER-RUNTIME-CONSUMPTION-001"', text)
        self.assertIn("GLOBAL_CONVERGENCE_TASK_IDS", text)
        self.assertIn("RUNTIME_PROFILE_MAP_TASK_ID", text)
        self.assertIn("GLOBAL_MEASUREMENT_TASK_ID", text)
        self.assertIn("STEGBROWSER_TASK_ID", text)
        self.assertIn("STEGBROWSER_RUNTIME_CONSUMPTION_TASK_ID", text)
