...
def write_manifest(root: Path, framework_id: str, *, source_version="official docs"):
    ...
    path.write_text(json.dumps({
        ...
        "boundary": {"execution_authority_claim": False, "compatibility_manifest_is_authority": False},
    }), encoding="utf-8")
...
def test_registry_sweep_classifies_each_framework_independently(tmp_path):
    ...
    result = module.build_registry_plan(
        registry=registry(), manifest_root=tmp_path,
    ...
    assert result["eligibility_counts"] == {"ROUNDTRIP_ELIGIBLE": 1, "RUNTIME_ENDPOINT_UNAVAILABLE": 1, "SOURCE_ONLY": 1}
    assert result["roundtrip_eligible_frameworks"] == ["alpha"]
    assert result["endpoint_binding_counts"] == {"EVIDENCE_QUALIFIED_ENDPOINT_BOUND": 1, "UNBOUND_NO_RUNTIME_ENDPOINT_REF": 2}
...
def test_bare_or_unqualified_endpoint_fails_closed(tmp_path):
    ...
    for framework_id in ("alpha", "beta", "blocked"):
        write_manifest(tmp_path, framework_id)
    result = module.build_registry_plan(
        registry=registry(), manifest_root=tmp_path,
        goal_task_id="MIR-CONNECTION-ROUNDTRIP-TECHNICAL-GUIDE-001", cosv_task_vector="50000000100000",
        endpoint_map={"alpha": "https://alpha.test/evaluate", "beta": {"runtime_endpoint_ref": "https://beta.test/evaluate"}},
    )
...
def test_missing_manifest_fails_only_that_framework(tmp_path):
    ...
    result = module.build_registry_plan(
        registry=registry(), manifest_root=tmp_path,
        goal_task_id="MIR-CONNECTION-ROUNDTRIP-TECHNICAL-GUIDE-001", cosv_task_vector="50000000100000",
        endpoint_map={"bindings": {"alpha": qualified_endpoint(), "beta": qualified_endpoint("https://beta.test/evaluate")}},
    )
    rows = {row.get("framework_id"): row for row in result["plans"]}
