...
class TestOrganizationSourceCoverage(unittest.TestCase):
    def test_merged_coverage_and_rights_boundary(self):
        ...
        self.assertEqual(c["counts"]["metadata_audited_repositories"],252)
        self.assertEqual(c["counts"]["metadata_audited_organizations"],14)
        self.assertEqual(c["counts"]["other_organizations_visibility_only"],0)
        self.assertEqual(c["counts"]["repository_entries"],252)
        self.assertEqual(sum(o["visible_repositories"] for o in c["organizations"] if o["license_metadata_coverage"]!="NOT_AUDITED"),252)
        self.assertEqual(r["tasks"][[t["task_id"] for t in r["tasks"]].index(TASK)],s)
        self.assertEqual(s["ecosystem_inventory"]["metadata_checked_entries"],252)
