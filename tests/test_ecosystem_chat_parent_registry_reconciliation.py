...
class EcosystemChatParentRegistryReconciliationTests(unittest.TestCase):
    ...
    def test_fresh_parent_fence_floor_is_strictly_above_terminal_recovery(self):
        retained_registry_admission = self.task["admission"]
        current_admission = self.fragment_task["admission"]
        handoff_activation = self.handoff["activation"]
        self.assertEqual(retained_registry_admission["minimum_fencing_token_exclusive"], 22)
        self.assertEqual(current_admission["minimum_fencing_token_exclusive"], 24)
        self.assertEqual(
            handoff_activation["minimum_fencing_token_exclusive"],
            current_admission["minimum_fencing_token_exclusive"],
        )
        self.assertGreater(
            current_admission["minimum_fencing_token_exclusive"],
