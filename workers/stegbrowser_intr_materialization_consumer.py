#!/usr/bin/env python3
"""StegBrowser consumer for the existing Universal InTr materialization path.

Reuses the StegVerse-002 event-ephemeral architecture: write-once admitted InTr
request -> non-authorizing consumer -> existing manifest-bound StegBrowser runner.
This module starts no listener, scheduler, WorkerCoordinator, or alternate runtime.
"""
...
def validate_request(request: Mapping[str, Any]) -> None:
    ...
    for field in ("materialization_id","operation_id","packet_id","payload_ref","transport_intent_hash","payload_hash","request_hash","node_genesis_receipt_ref"):
...
def scrubbed_env(env: Mapping[str, str] | None = None) -> dict[str, str]:
    ...
    keep = {"PATH","HOME","LANG","LC_ALL","SSL_CERT_FILE","SSL_CERT_DIR","XDG_STATE_HOME","XDG_CONFIG_HOME","LOCALAPPDATA","STEGVERSE_STEGOS_SOURCE_ROOT","STEGVERSE_EPHEMERAL_RUNTIME_BASE"}
...
def consume_one(source_root: Path, runtime_root: Path, materialization_id: str, *, runner=subprocess.run, env: Mapping[str, str] | None = None) -> dict[str, Any]:
    ...
    request = load(request_path); validate_request(request)
    ...
    node_id, interlock_id = ingress.get("node_id"), ingress.get("interlock_id")
    require(isinstance(node_id, str) and node_id and isinstance(interlock_id, str) and interlock_id, "stegbrowser_node_interlock_binding_missing")
    ...
    receipt = {
        ...
        "workercoordinator_claim_fence_observed": boundary.get("workercoordinator_claim_fence_observed") is True,
        "organization_local_intr_ingress_receipt_verified": boundary.get("organization_local_intr_ingress_receipt_verified") is True,
        "authentic_intr_ingress_observed": boundary.get("authentic_intr_ingress_observed") is True,
        "claim_id": boundary.get("claim_ref"),
        "fencing_token": boundary.get("fence_ref"),
    ...
    if out.exists(): require(load(out) == receipt, "stegbrowser_consumption_write_once_collision")
    ...
    latest.parent.mkdir(parents=True, exist_ok=True); latest.write_text(raw, encoding="utf-8")
    return receipt


def main() -> int:
    p = argparse.ArgumentParser(); p.add_argument("--source-root", type=Path, default=ROOT); p.add_argument("--runtime-root", type=Path, required=True); p.add_argument("--materialization-id", required=True); args=p.parse_args()
