...
def _load_installer():
    path = ROOT / "scripts" / "install_sovereign_heartbeat_carrier.py"
    spec = importlib.util.spec_from_file_location("install_sovereign_heartbeat_carrier", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load canonical sovereign carrier-only installer")
...
def _response(state: str, transition_id: str, sequence: int, *, blocker=None, evidence_refs=None):
    return {
        ...
        "checkpoint_ref": "receipts/sovereign-host/carrier-activation.latest.json",
...
def main() -> int:
    ...
    try:
        ...
        base = installer.base
        target = base.default_runtime_root()
        receipt = installer.install_carrier(ROOT, target)
        ...
        if receipt.get("activation_scope") != "CARRIER_ONLY":
            raise RuntimeError("installer did not preserve carrier-only activation scope")
        if receipt.get("worker_start_attempted") is not False:
            raise RuntimeError("carrier-only startup unexpectedly attempted WorkerCoordinator startup")
        if receipt.get("worker_runtime_dependency_for_carrier_start") is not False:
            raise RuntimeError("carrier startup still depends on WorkerCoordinator")
        ...
        if receipt.get("heartbeat_reference_frequency_hz") != 100.0:
            raise RuntimeError("resident carrier frequency is not 100 Hz")
        if receipt.get("network_fetch_required") is not False:
            raise RuntimeError("carrier startup unexpectedly requires network fetch")
        for key in (
            "third_party_process_host_required",
            "third_party_scheduler_required",
            "third_party_deployment_required",
            "github_runtime_dependency",
        ):
            if receipt.get(key) is not False:
                raise RuntimeError(f"carrier startup unexpectedly requires {key}")
        ...
        if receipt.get("credential_authority") != "TV/TVC":
            raise RuntimeError("resident heartbeat startup changed credential authority")
        ...
        blocker = {
            ...
            "next_solution_action": "RUN_CARRIER_ONLY_NATIVE_INSTALLER_ON_ADMITTED_RESIDENT_HOST",
            "machine_observable_release_condition": "carrier-activation.latest.json records carrier_active=true for canonical engine_v13 oscillator phase-driven service",
    ...
    evidence = [
        ...
        "receipts/sovereign-host/carrier-activation.latest.json",
    ]
