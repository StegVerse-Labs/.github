...
Third-party availability is never a StegVerse blocker. Cloudflare is one
eligible deployment path, not production authority. If its credentials or
capability are unavailable, this worker remains ACTIVE and emits a required
workaround contract so orchestration can pursue a StegVerse-owned/federated
rendezvous carrier or another admitted deployment path.
"""
...
def request_json(url: str, *, token: str | None = None, timeout: int = 20) -> tuple[int, dict]:
    headers = {"accept": "application/json", "user-agent": "StegVerse-Heartbeat-Rendezvous-Worker/2"}
    if token:
...
def response(state: str, transition: str, epoch: int, receipt_ref: str, *, next_transition: str | None, retry_latest: int | None, blocker: dict | None = None) -> dict:
    result = {
        ...
        "evidence_refs": [receipt_ref, "control/blocker-resolution-policy.json"],
        "cost_observation": {
...
def third_party_workaround(problem: str) -> dict:
    return {
        ...
        "workaround_candidates": [
            "Deploy the same pinned rendezvous source on a StegVerse-owned/federated sovereign carrier.",
            "Expose the rendezvous through an already-authorized StegVerse gateway without Cloudflare deployment authority.",
            "Use another admitted deployment surface while preserving the same route contract and live acceptance tests."
        ],
        "next_solution_action": "Select and activate the highest-ranked admitted non-provider-dependent rendezvous path; do not wait on provider credentials."
    }


def internal_blocker(problem: str, next_action: str) -> dict:
    return {
...
def main() -> int:
    ...
    base_receipt = {
        "schema": "stegverse.steggate-rendezvous-worker-receipt/v0.2",
        "task_id": TASK_ID,
    ...
    if not token or not account_id:
        blocker = third_party_workaround("Cloudflare credential values are absent from this execution environment.")
        receipt = dict(base_receipt)
        receipt.update({
            "state": "ACTIVE",
            "transition": "THIRD_PARTY_WORKAROUND_REQUIRED",
            "provider_path": "cloudflare",
            "provider_condition": "CREDENTIAL_VALUES_ABSENT",
            "blocker": blocker,
            "next_action": blocker["next_solution_action"]
        })
        write_receipt(receipt_path, receipt)
        json.dump(response("ACTIVE", "THIRD_PARTY_WORKAROUND_REQUIRED", epoch, receipt_ref, next_transition="ALTERNATE_RENDEZVOUS_PATH", retry_latest=epoch + 1, blocker=blocker), sys.stdout)
        sys.stdout.write("\n")
        return 0
    ...
    if status != 200 or capability.get("success") is not True:
        blocker = third_party_workaround("The available Cloudflare account/token pair cannot perform the admitted deployment.")
        receipt = dict(base_receipt)
        receipt.update({
            "state": "ACTIVE",
            "transition": "THIRD_PARTY_WORKAROUND_REQUIRED",
            "provider_path": "cloudflare",
            "provider_condition": "CREDENTIAL_CAPABILITY_REJECTED",
    ...
    if observed_blob != SITE_WORKER_GIT_BLOB_SHA:
        next_action = "Reconcile authorization to the reviewed canonical Site source or use the already-pinned authorized blob from a trusted StegVerse-local source."
        blocker = internal_blocker("Authorized rendezvous source identity differs from the currently fetched source.", next_action)
        receipt = dict(base_receipt)
        receipt.update({
            "state": "BLOCKED",
        ...
        write_receipt(receipt_path, receipt)
        json.dump(response("BLOCKED", "SOURCE_DRIFT_REVIEW_REQUIRED", epoch, receipt_ref, next_transition="SOURCE_RECONCILIATION", retry_latest=epoch + 1, blocker=blocker), sys.stdout)
        sys.stdout.write("\n")
        return 0
    ...
    with tempfile.TemporaryDirectory(prefix="steggate-rendezvous-") as td:
        ...
        if proc.returncode != 0:
            blocker = third_party_workaround("Cloudflare deployment command failed; provider failure cannot halt rendezvous activation.")
            receipt = dict(base_receipt)
            receipt.update({
                "state": "ACTIVE",
                "transition": "THIRD_PARTY_WORKAROUND_REQUIRED",
    ...
    if not accepted:
        ...
        receipt.update({
            ...
            "next_action": "Repair the rendezvous implementation or activate an alternate admitted rendezvous carrier; deployment does not imply acceptance."
        })
        write_receipt(receipt_path, receipt)
        json.dump(response("ACTIVE", "LIVE_ACCEPTANCE_RETRY_REQUIRED", epoch, receipt_ref, next_transition="LIVE_ACCEPTANCE_RETRY", retry_latest=epoch + 1), sys.stdout)
        sys.stdout.write("\n")
        return 0
