...
def roots()->list[Path]:
    return [ROOT/"workloads"/"stegfin-governance",Path.home()/".stegverse"/"workloads"/"stegfin-governance",Path("/var/lib/stegverse/workloads/stegfin-governance")]
...
def main()->int:
    ...
    root=find_root()
    if root is None:
        b=blocker("The released StegFin activation capsule is not locally materialized on the sovereign carrier.","Materialize the released StegFin workload locally.","find_root resolves run_sovereign_trading_activation_round.py and the canonical StegFin handoff")
        durable={"schema":"stegverse.stegfin-sovereign-trading-worker-receipt/v0.2","task_id":TASK,"heartbeat_epoch":epoch,"claim_id":claim,"fencing_token":fence,"state":"BLOCKED","transition_id":"STEGFIN_SOVEREIGN_CAPSULE_NOT_MATERIALIZED","github_token_required":False,"wallet_signing_authority":False,"transaction_broadcast_authority":False,"custody_authority":False,"scale_up_authority":False,"blocker":b}; atomic_write(RECEIPT,durable); json.dump(response("BLOCKED",durable["transition_id"],1,"STEGFIN_SOVEREIGN_TRADING_ACTIVATED",b),sys.stdout); print(); return 0
