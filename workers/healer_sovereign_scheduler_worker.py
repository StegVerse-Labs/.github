...
def sv002_observation_gateway_projection() -> dict[str, str]:
    ...
    disabled = {
        "STEGVERSE_SV002_OBSERVE_ENABLED": "false",
        "STEGVERSE_SV002_OBSERVE_UPSTREAM": "",
    }
    if not path.is_file():
        return disabled
    try:
    ...
    if not isinstance(value, dict):
        return disabled
    if value.get("schema") != "stegverse.sv002-public-observation-route-config/v1":
        return disabled
    if value.get("credential_authority") != "TV/TVC":
        return disabled
    if value.get("github_token_runtime_authority") != "NONE":
    ...
    if not isinstance(port, int) or port < 1024 or port > 65535:
        return disabled
    return {
        "STEGVERSE_SV002_OBSERVE_ENABLED": "true",
        "STEGVERSE_SV002_OBSERVE_UPSTREAM": f"http://127.0.0.1:{port}/intr/sv002-observe",
    }
