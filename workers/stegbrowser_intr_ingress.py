#!/usr/bin/env python3
"""StegBrowser profile adapter for the existing Universal InTr listener.

No listener is created here. This adapter reuses the existing transport header
validation and write-once materialization directories, validates a Node-bound
trigger, writes one StegBrowser INGRESS_ADMITTED receipt, and dispatches the
...
from consume_stegbrowser_intr_materialization_request import (  # noqa: E402
    DESTINATION,
    DOWNSTREAM_OWNER,
    INGRESS_SCHEMA,
    scrubbed_env,
...
PROFILE_ID = "StegBrowser:ManifestExecution"
...
def admit(*, runtime_root: Path, body: bytes, headers: Mapping[str, str]) -> dict[str, Any]:
    ...
    require(transport.get("origin") == transport_boundary.ORIGIN_NODE, "stegbrowser_requires_node_origin")
    try:
    ...
    process = subprocess.Popen(
        [
            sys.executable,
            str(ROOT / "scripts" / "consume_stegbrowser_intr_materialization_request.py"),
...
__all__ = ["DESTINATION", "DOWNSTREAM_OWNER", "PROFILE_ID", "INGRESS_SCHEMA", "is_stegbrowser", "admit"]
