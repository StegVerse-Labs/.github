#!/usr/bin/env python3
"""Bounded far-side ingress for SV002 public-observation materialization requests.

The ingress accepts only a registered StegOS Node outbox trigger. It persists the
exact non-authorizing materialization request and an ingress receipt. It does not
execute the receiver, grant a claim/fence, or claim receiver readiness.
"""
...
from consume_sv002_intr_materialization_request import validate_request

MAX_REQUEST_BYTES = 256 * 1024
INGRESS_PATH = "/intr/materialization"
REQUEST_DIR_REL = Path("sv002-intr-materialization")
RECEIPT_DIR_REL = Path("receipts/sovereign-network/sv002-intr-materialization-ingress")
LATEST_REL = Path("receipts/sovereign-network/sv002-intr-materialization-ingress.latest.json")
RECEIPT_SCHEMA = "stegverse.sv002-intr-materialization-ingress/v1"
...
class SV002InTrIngressError(ValueError):
    pass


def _require(condition: bool, reason: str) -> None:
    if not condition:
        raise SV002InTrIngressError(reason)
...
def validate_transport_headers(headers: Mapping[str, str], body: bytes) -> dict[str, str]:
    ...
    _require(origin == ORIGIN_NODE, "transport_origin_header_invalid")
    _require(not authorization_id, "node_outbox_cannot_claim_tvc_authorization")
    _require(content_type == "application/json", "content_type_not_supported")
    _require(
        len(supplied_hash) == 64 and all(ch in "0123456789abcdef" for ch in supplied_hash),
        "payload_sha256_header_invalid",
    )
    _require(supplied_hash == _sha256(body), "payload_sha256_header_mismatch")
    return {"transport": transport, "origin": origin, "payload_sha256": supplied_hash}
...
def _validate_node_outbox_entry(entry: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _require(
        entry.get("state") == "LOCAL_OUTBOX_PENDING_NETWORK_DELIVERY",
        "node_outbox_state_invalid",
    )
    _require(
        re.fullmatch(r"SV-NODE-[a-f0-9]{24}", str(entry.get("node_id", ""))) is not None,
        "node_id_invalid",
    )
    _require(
        re.fullmatch(r"SV-IL-[a-f0-9]{24}", str(entry.get("interlock_id", ""))) is not None,
        "interlock_id_invalid",
    )
    for field in (
        "network_delivery_observed",
        "runtime_materialization_observed",
        "receiver_receipt_observed",
        "tvc_receipt_observed",
    ):
        _require(entry.get(field) is False, f"node_outbox_{field}_must_be_false")
    ...
    body = dict(entry)
    body.pop("outbox_entry_hash", None)
    ...
    for field in (
        ...
        _require(entry.get(field) == request.get(field), f"node_outbox_{field}_mismatch")
    return request


def extract_materialization(payload: Any) -> tuple[dict[str, Any], dict[str, Any]]:
    _require(
        isinstance(payload, dict) and payload.get("schema") == NODE_TRIGGER_SCHEMA,
...
def _receipt_binding_matches(
    receipt: Mapping[str, Any], request: Mapping[str, Any], source: Mapping[str, Any]
) -> bool:
    return all(
        (
...
def admit_materialization(
    *, runtime_root: Path, body: bytes, headers: Mapping[str, str]
    ...
    try:
        ...
        raise SV002InTrIngressError("request_json_invalid") from exc
    request, source = extract_materialization(payload)
    ...
    if receipt_path.exists():
        try:
            ...
            raise SV002InTrIngressError("existing_ingress_receipt_invalid") from exc
...
class IngressServer(ThreadingHTTPServer):
    def __init__(
        self, address: tuple[str, int], runtime_root: Path, max_requests: int
    ):
...
class IngressHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            receipt = admit_materialization(
                runtime_root=self.server.runtime_root, body=body, headers=self.headers
            )
        except Exception as exc:
...
def serve(
    *,
    ...
    if tls_cert or tls_key:
        ...
        server.server_close()
        raise SV002InTrIngressError("non_loopback_ingress_requires_tls")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Serve bounded SV002 InTr materialization ingress.")
    ...
    print(
        json.dumps(
            {
                "schema": "stegverse.sv002-intr-materialization-ingress-listener/v1",
