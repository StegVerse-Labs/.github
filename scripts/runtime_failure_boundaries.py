...
The classifier is observational only. It does not grant execution, claim/fence,
Interlock/InTr, credential, custody, publication, or completion authority.
"""
...
# Existing lane resume labels mapped onto the canonical ten-stage loop.
RESUME_STAGE_ALIASES: dict[str, int] = {
...
PASS_STATES = {
    "PASS", "PASSED", "SATISFIED", "COMPLETED", "ALREADY_CONSUMED", "OBSERVED",
    "ADMITTED", "BOUND", "EXECUTED", "RECONSTRUCTED", "VERIFIED",
}
...
TERMINAL_SUCCESS_STATES = {
    ...
    "PROFILE_BOUND_PARENT_CHAIN_PRESENT_REEXECUTION_READY",
}
...
def _observation_failed(value: Any) -> bool:
    ...
    if state in FAIL_STATES:
        return True
    return any(token in state for token in ("FAIL", "ERROR", "MISSING", "UNOBSERVED", "NOT_", "DENIED", "REFUSED", "UNAVAILABLE", "PENDING"))
...
def _stage_from_resume(resume_stage: Any) -> int:
    ...
    if "CLAIM" in value or "FENCE" in value:
        return 4
    if "INTR" in value or "INTERLOCK" in value:
        return 5
    if "LEASE" in value or "PROVIDER" in value or "TRANSPORT" in value:
        return 6
    if "RECONSTRUCTION" in value or "MASTER_RECORD" in value:
        return 9
    if "PROPAGATION" in value or "PUBLICATION" in value:
        return 10
    if "RECEIPT" in value or "CUSTODY" in value:
        return 8
    if "EXECUTION" in value or "ANALYSIS" in value or "INVOCATION" in value or "REEXECUTION" in value:
        return 7
    if "REQUEST" in value or "CONSUMPTION" in value or "INGRESS" in value:
        return 3
    return 1
...
def annotate_lane_outcome(outcome: Mapping[str, Any]) -> dict[str, Any]:
    """Attach an ordered trace and exact first-failure response to one lane outcome.

    A child may provide `stage_observations` keyed by canonical stage name. When
    present, those observations take precedence and the earliest failing/unobserved
    boundary wins. Otherwise the existing canonical `resume_stage` is used as the
    first unresolved boundary, preserving current non-regression semantics.
    """
    result = dict(outcome)
    ...
    if isinstance(explicit, Mapping):
        for stage in STAGES:
            ...
            if isinstance(observation, Mapping):
                ...
                observed_state = observation if observation is not None else "NOT_REPORTED"
                observed_reason = None
                observed_ref = None
            failed = _observation_failed(observed_state) if observation is not None else False
        ...
        for stage in STAGES:
            if success:
                state = "PASSED_OR_NOT_REQUIRED_BY_EXISTING_EVIDENCE"
                failed = False
            elif stage["index"] < resume_index:
                state = "PASSED_BY_EXISTING_EVIDENCE"
                failed = False
            elif stage["index"] == resume_index:
                state = "FIRST_UNRESOLVED:" + str(raw_state or result.get("resume_stage") or "UNKNOWN")
                failed = True
