...
def main() -> int:
    ...
    require(manifest.get("goal_task_id") == PARENT_TASK_ID, "manifest_parent_goal_mismatch")
    ...
    protocol_resolved = bool(callable_value and "CanonicalWork:Coordination" in profiles)
    ...
    observation = {
        ...
        "observation_source": "LIVE_SHARED_UNIVERSAL_INTR_PROFILE",
    ...
    if SOURCE_REFRESH_RT in resolution.get("selected_reusable_tasks", []):
        refresh_receipt = run_source_refresh(source_root, runtime_root)

    result = {
        "schema": "stegverse.stegbrowser-runtime-connection-a1-a2-observation/v1",
        "state": "A1_OBSERVED_A2_SELECTION_APPLIED" if callable_value else "A1_OBSERVED_NOT_CALLABLE",
        "task_id": TASK_ID,
        "cosv": COSV,
        ...
        "refreshable": refreshable_value,
        "applicable_protocol_resolved": protocol_resolved,
        "selected_reusable_tasks": resolution.get("selected_reusable_tasks", []),
        "source_refresh_receipt": refresh_receipt,
        "admitted_execution_surface_materialized": bool(callable_value and (not refreshable_value or refresh_receipt is not None)),
        ...
        "authority_effect": "NONE_OBSERVATION_SELECTION_AND_LOCAL_MATERIALIZATION_ONLY",
    }
    out = runtime_root / "receipts/sovereign-host/stegbrowser-runtime-connection-a1-a2.latest.json"
