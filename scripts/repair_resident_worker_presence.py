...
SAFE_ENV = {
    ...
    "STEGVERSE_LLM_ADAPTER_ROOT", "STEGVERSE_UNIVERSAL_INTR_INGRESS_URL", "STEGVERSE_TVC_ROOT", "STEGVERSE_TV_ROOT",
...
def _clean_env(runtime_root: Path) -> dict[str, str]:
    ...
    for name in list(env):
        upper = name.upper()
        if name not in CANONICAL_CUSTODY_ENV and any(part in upper for part in FORBIDDEN_NAME_PARTS):
            env.pop(name, None)
    return env
...
def ensure_worker_presence(runtime_root: Path, *, carrier_pid: int, interval_ms: float = 10.0, timeout: float = 3.0) -> dict[str, Any]:
    ...
    if _alive(existing_worker_pid):
        ...
        stale_worker_reason = (
            "WORKER_CYCLE_STALE" if structural_task_capable else "WORKER_NOT_TASK_CAPABLE"
        )
