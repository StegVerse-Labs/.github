...
def consume(runtime_root: Path, *, environ: Mapping[str, str] | None = None, runner=subprocess.run) -> dict[str, Any]:
    ...
    base = {
        ...
        "authority_effect": "NONE_RESIDENT_RESEAL_EXECUTION_ONLY",
        "tvc_root_observation": tvc_observation,
        "source_custody_receipt_present": source.is_file(),
        "target_custody_receipt_present_before": target.is_file(),
        "resident_seal_activation_receipt_present": activation.is_file(),
        "resident_private_key_present": private_key.is_file(),
    }
    ...
    if target.is_file():
        ...
        return receipt
    missing = []
    for label, path in (("SOURCE_CUSTODY", source), ("RESIDENT_SEAL_ACTIVATION", activation), ("RESIDENT_PRIVATE_KEY", private_key)):
        if not path.is_file():
    ...
    ok = (
        ...
        and result.get("plaintext_returned") is False
        and result.get("plaintext_persisted") is False
        and result.get("plaintext_logged") is False
        and result.get("plaintext_hashed") is False
        and result.get("provider_operation_authority") is False
        and target.is_file()
    )
    receipt = {
        **base,
        ...
        "secret_free_reseal_result": result if ok else None,
        "target_custody_receipt_present_after": target.is_file(),
        "stdout_secret_material_retained": False,
        "stderr_secret_material_retained": False,
    }
    _atomic_json(previous_path, receipt)
    return receipt
