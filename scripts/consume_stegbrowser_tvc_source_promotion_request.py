...
ENTRYPOINT = "scripts/consume_stegbrowser_tvc_source_promotion_request.py"
...
def consume(source_root: Path, runtime_root: Path) -> dict[str, Any]:
    ...
    receipt = {
        ...
        "retry_allowed": state == "HANDOFF_READY",
...
def main() -> int:
    ...
    return 0 if result.get("state") in {"STAGED", "ALREADY_STAGED", "RESTAGED_EXACT_SOURCE", "HANDOFF_READY"} else 1
