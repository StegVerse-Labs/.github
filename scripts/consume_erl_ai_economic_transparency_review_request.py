...
REQUEST_REL=Path("control/resident-execution-request.d/erl-ai-economic-transparency-review-001.json")
CONSUMPTION_REL=Path("receipts/sovereign-host/erl-ai-economic-transparency-review-resident-execution-request-consumption.latest.json")
TARGET_TASK="SHWP-ERL-AI-ECON-TRANSPARENCY-REVIEW-001"
...
NONSECRET_ENV=("PATH","HOME","LANG","LC_ALL","XDG_STATE_HOME","XDG_CONFIG_HOME","LOCALAPPDATA","STEGVERSE_SOVEREIGN_NODE","STEGVERSE_HEARTBEAT_ROOT","STEGVERSE_HEARTBEAT_SOURCE_ROOT")
...
def validate_request(r):
    ...
    for k,e in required.items():
        if r.get(k)!=e: raise RuntimeError(f"ERL review resident request {k} mismatch")
...
def clean_env(source: Mapping[str,str] | None=None):
    ...
    if hosted:
        raise RuntimeError("hosted environment may not consume ERL independent-review request: "+",".join(sorted(hosted)))
...
def consume(source_root,runtime_root,*,runner=subprocess.run,env=None):
    ...
    if not p.is_file():
        return {"schema":"stegverse.erl-ai-economic-transparency-review-resident-execution-request-consumption/v1","state":"NO_REQUEST","runtime_execution_attempted":False,"authority_effect":"NONE"}
    r=load(p); validate_request(r); h=stable(r)
    if consumed(runtime,r,h):
        return {"schema":"stegverse.erl-ai-economic-transparency-review-resident-execution-request-consumption/v1","state":"ALREADY_CONSUMED","request_id":r["request_id"],"request_sha256":h,"runtime_execution_attempted":False,"authority_effect":"NONE"}
    entry=runtime/TARGET_ENTRYPOINT
    if not entry.is_file(): raise RuntimeError("ERL review resident entrypoint missing")
    ...
    receipt={
        "schema":"stegverse.erl-ai-economic-transparency-review-resident-execution-request-consumption/v1",
    ...
    q=runtime/CONSUMPTION_REL
    q.parent.mkdir(parents=True,exist_ok=True)
    q.write_text(json.dumps(receipt,indent=2,sort_keys=True)+"\n",encoding="utf-8")
    return receipt
...
def main():
    ...
    a=ap.parse_args()
    r=consume(a.source_root,a.runtime_root)
    print(json.dumps(r,sort_keys=True))
    return 0 if r["state"] in {"NO_REQUEST","ALREADY_CONSUMED","ATTEMPT_RECORDED"} else 1
