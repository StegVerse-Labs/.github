#!/usr/bin/env python3
from __future__ import annotations
import json, sys
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
RECORDS = ROOT / "data" / "canonical-task-records"
ACTIVEISH = {"ACTIVE", "CHECKED_OUT", "CLAIMED_INTEGRATION", "HANDOFF_READY_RUNTIME_PROOF_PENDING", "BLOCKED_RUNTIME_ACTIVATION"}


def load_records():
    out = {}
    for p in RECORDS.glob("*.json"):
        try:
            r = json.loads(p.read_text())
        except Exception:
            continue
        tid = str(r.get("task_id") or "").strip()
        if tid:
            out[tid] = r
    return out


def handoff(r):
    refs = r.get("handoff_projection_refs") or r.get("source_refs") or []
    for x in refs:
        if isinstance(x, str) and "HANDOFF" in x.upper():
            return x
    return None


def overlap(a, b):
    at, bt = a.get("targets") or {}, b.get("targets") or {}
    repos = sorted(set(at.get("repositories") or []) & set(bt.get("repositories") or []))
    comps = sorted(set(at.get("components") or []) & set(bt.get("components") or []))
    lineage = bool({a.get("task_id"), a.get("parent_task_id"), a.get("root_correlation_id")} & {b.get("task_id"), b.get("parent_task_id"), b.get("root_correlation_id")})
    adjacent = b.get("task_id") in (a.get("adjacent_task_refs") or []) or a.get("task_id") in (b.get("adjacent_task_refs") or [])
    return repos, comps, lineage, adjacent


def main():
    req = json.load(sys.stdin)
    tid = str(req.get("task_id") or "").strip()
    records = load_records()
    r = records.get(tid)
    if not r:
        print(json.dumps({"schema":"stegverse.task-registry-checkin-disposition/v1","task_id":tid,"disposition":"STOP_NOT_REGISTERED","session_action":"END_OR_REGISTER_BEFORE_MUTATION","authority_effect":"NONE"}, sort_keys=True))
        return

    state = str(r.get("coordination_state") or "").upper()
    checkout = str(r.get("checkout_state") or "").upper()
    continuation = r.get("continuation_task_id")
    if state in {"SUPERSEDED","RETIRED","INACTIVE"} or checkout in {"SUPERSEDED","COMPLETED","RETIRED"}:
        print(json.dumps({"schema":"stegverse.task-registry-checkin-disposition/v1","task_id":tid,"disposition":"STOP_SUPERSEDED" if continuation else "STOP_INACTIVE","continuation_task_id":continuation,"continuation_handoff":handoff(records.get(continuation,{})) if continuation else None,"session_action":"END_SESSION_AND_CONTINUE_ONLY_AT_RETURNED_TASK" if continuation else "END_SESSION","authority_effect":"NONE"}, sort_keys=True))
        return

    collisions=[]
    for oid, o in records.items():
        if oid == tid: continue
        os = str(o.get("coordination_state") or "").upper()
        oc = str(o.get("checkout_state") or "").upper()
        if os not in ACTIVEISH and oc not in ACTIVEISH and oc != "CHECKED_OUT":
            continue
        repos, comps, lineage, adjacent = overlap(r,o)
        if repos or comps or lineage or adjacent:
            collisions.append({"task_id":oid,"handoff":handoff(o),"coordination_state":os,"checkout_state":oc,"overlap":{"repositories":repos,"components":comps,"lineage":lineage,"adjacent":adjacent}})

    hard=[c for c in collisions if c["checkout_state"]=="CHECKED_OUT" and (c["overlap"]["components"] or c["overlap"]["lineage"])]
    disposition = "STOP_COLLISION" if hard else ("COORDINATE_CONVERGENCE" if collisions else "CONTINUE")
    action = "END_SESSION_AND_CONTINUE_IN_RETURNED_COLLISION_OWNER" if hard else ("COORDINATE_BEFORE_MUTATION" if collisions else "CONTINUE_CURRENT_TASK")
    print(json.dumps({"schema":"stegverse.task-registry-checkin-disposition/v1","task_id":tid,"task_handoff":handoff(r),"disposition":disposition,"session_action":action,"collision_candidates":collisions,"hard_collision_task_ids":[c["task_id"] for c in hard],"authority_effect":"NONE"}, sort_keys=True))

if __name__ == "__main__":
    main()
