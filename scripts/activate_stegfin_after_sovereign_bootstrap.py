...
def child_env(env: dict[str, str] | None = None) -> dict[str, str]:
    values = dict(os.environ if env is None else env)
    # Installer needs only local process/bootstrap environment. Credentials are
    # intentionally not forwarded; TV/TVC remains the authority owner, but this
    # integration has credential_requirement=NONE.
