...
HOSTED_ENV_VARS = ("GITHUB_ACTIONS", "CI", "RENDER", "RENDER_SERVICE_ID", "VERCEL", "CF_PAGES", "CLOUDFLARE_WORKERS")
REQUIRED_PREDICATES = (
...
def activate(root: Path, *, proof_path: Path, bootstrap_receipt: Path, node_marker: Path,
    ...
    if not installer.is_file() or not executor.is_file():
        ...
        atomic_write(integration_receipt, result)
        return result
    completed = runner([sys.executable, str(installer), "--root", str(root)], check=False, capture_output=True,
                       text=True, timeout=180, env=child_env(env))
    result["installer_returncode"] = completed.returncode
    activation = load_json(executor_activation_receipt)
    active = bool(
        completed.returncode == 0 and activation and activation.get("active") is True
