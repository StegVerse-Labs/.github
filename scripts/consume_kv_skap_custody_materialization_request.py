#!/usr/bin/env python3
"""Consume admitted KV -> SKAP Vault Universal InTr ciphertext custody requests.

The organization .github boundary owns transport admission/dispatch only.
TVC remains the double-Interlock and credential/custody authority. The same
resident STEGVERSE_KV_ROOT used by DEVICE_KV is used for _Vault/SKAP custody.
"""
...
def validate_request(request: dict[str, Any]) -> None:
    ...
    for key in ("transport_intent_hash", "payload_hash", "request_hash"):
        ...
        if not isinstance(value, str) or len(value) != 71 or not value.startswith("sha256:"):
            raise KVSkapMaterializationError(key + "_invalid")
    ...
    if request.get("payload_ref") != "inline://materialization_request.sealed_capsule":
        raise KVSkapMaterializationError("payload_ref_invalid")
    if request.get("payload_hash") != sha_uri(capsule):
        raise KVSkapMaterializationError("sealed_capsule_payload_hash_mismatch")
    body = dict(request)
...
def consume_one(source_root: Path, runtime_root: Path, materialization_id: str, env: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    capsule = request["sealed_capsule"]
    ...
    if sha_uri(raw_packet) != request["payload_hash"]:
        raise KVSkapMaterializationError("exact_capsule_hash_mismatch")
    ...
    result_body = {
        ...
        "exact_ciphertext_readback_verified": Path(persisted["credential_path"]).read_bytes() == raw_packet,
        "secret_plaintext_present": False,
        "kv_decryption_authority": False,
