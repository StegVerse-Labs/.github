...
REQUEST_REL=Path("control/resident-execution-request.d/sv011-phase5-boundary-001.json")
CONSUMPTION_REL=Path("receipts/sovereign-host/sv011-phase5-resident-execution-request-consumption.latest.json")
TARGET_TASK="SHWP-SV011-PHASE5-BOUNDARY-001"
...
NONSECRET_ENV=("PATH","HOME","LANG","LC_ALL","XDG_STATE_HOME","XDG_CONFIG_HOME","LOCALAPPDATA","STEGVERSE_SOVEREIGN_NODE","STEGVERSE_HEARTBEAT_ROOT","STEGVERSE_HEARTBEAT_SOURCE_ROOT","STEGVERSE_SV011_ORG_ROOT")
...
def validate_request(r):
    ...
    for k,e in required.items():
        if r.get(k)!=e: raise RuntimeError(f"SV-011 resident request {k} mismatch")
...
def clean_env(source=None):
    ...
    if hosted: raise RuntimeError("hosted environment may not consume SV-011 sovereign request: "+",".join(sorted(hosted)))
...
def consume(source_root,runtime_root,*,runner=subprocess.run,env=None):
    ...
    if not p.is_file(): return {"schema":"stegverse.sv011-resident-execution-request-consumption/v1","state":"NO_REQUEST","runtime_execution_attempted":False,"authority_effect":"NONE"}
    r=load(p); validate_request(r); h=stable(r)
    if consumed(runtime,r,h): return {"schema":"stegverse.sv011-resident-execution-request-consumption/v1","state":"ALREADY_CONSUMED","request_id":r["request_id"],"request_sha256":h,"runtime_execution_attempted":False,"authority_effect":"NONE"}
    entry=runtime/TARGET_ENTRYPOINT
    if not entry.is_file(): raise RuntimeError("SV-011 resident entrypoint missing")
    ...
    receipt={"schema":"stegverse.sv011-resident-execution-request-consumption/v1","state":"ATTEMPT_RECORDED" if valid else "FAIL_CLOSED","request_id":r["request_id"],"request_sha256":h,"task_id":TARGET_TASK,"mode":TARGET_MODE,"command":command,"execution_returncode":done.returncode,"execution_result_observed":isinstance(result,dict),"execution_result":result,"bridge_contract_valid":valid,"runtime_execution_attempted":True,"request_granted_authority":False,"activation_claimed":False,"heartbeat_grants_execution_authority":False,"github_token_required":False,"github_token_runtime_authority":"NONE","credential_authority":"TV/TVC","network_source_fetch_performed":False,"authority_effect":"NONE_REQUEST_ONLY"}
