...
def _receipt_specs(values: Mapping[str, str]) -> dict[str, tuple[Path, dict[str, Any]]]:
    ...
    return {
        "SV-DN1-SOURCE-MATERIALIZATION-001": (
            source_root / "receipts" / "latest.json",
            {
                "state": "COMPLETE",
        ...
        "SV-DN1-RESIDENT-OBSERVER-001": (
            resident_root / "receipts" / "latest.json",
            {
                "state": "COMPLETE",
        ...
        "SV-DN1-INTR-RUNTIME-001": (
            intr_root / "receipts" / "latest.json",
            {
                "state": "COMPLETE",
...
def validate_durable_receipt(task_id: str, values: Mapping[str, str]) -> dict[str, Any]:
    ...
    return {
        ...
        "receipt_schema": receipt.get("schema") or receipt.get("schema_version"),
...
def execute_chain(
    ...
    *,
    ...
    if hosted:
        raise RuntimeError(
            "hosted execution cannot produce sovereign SV-DN-1 runtime evidence: "
            + ",".join(hosted)
        )
    ...
    if not carrier.is_file():
        return {
            ...
            "state": "HANDOFF_READY",
            "transition_id": "SV_DN1_SOVEREIGN_CARRIER_REFERENCE_PENDING",
            "completed_tasks": [],
            "next_task": TASKS[0],
    ...
    for task_id in TASKS:
        ...
        # A fragment may not have been admitted into the mutable registry until
        # the targeted cycle. If already COMPLETED, never reacquire it.
        ...
        if row is not None and row.get("state") in {"ACTIVE", "BLOCKED"}:
            return {
                "schema": "stegverse.sv-dn1.sovereign-execution-chain/v1",
                "state": "HANDOFF_READY",
                "transition_id": "SV_DN1_EXISTING_TASK_LIFECYCLE_MUST_RESOLVE",
        ...
        cycle_result = _parse_last_json(completed.stdout)
        ...
        if completed.returncode != 0 or state != "COMPLETED":
            ...
            return {
                "schema": "stegverse.sv-dn1.sovereign-execution-chain/v1",
                "state": "HANDOFF_READY",
                "transition_id": "SV_DN1_CHAIN_STEP_NOT_TERMINAL",
                "completed_tasks": completed_tasks,
                "next_task": task_id,
                "task_state": state,
                "runtime_root": str(runtime),
        ...
        try:
            ...
            return {
                "schema": "stegverse.sv-dn1.sovereign-execution-chain/v1",
                "state": "BLOCKED",
                "transition_id": "SV_DN1_CHAIN_DURABLE_RECEIPT_MISMATCH",
                "completed_tasks": completed_tasks,
                "next_task": task_id,
                "runtime_root": str(runtime),
                "refresh_receipt": refresh_receipt,
    ...
    receipt = {
        "schema": "stegverse.sv-dn1.sovereign-execution-chain/v1",
        "state": "COMPLETE",
        "transition_id": "SV_DN1_SOVEREIGN_FIRST_ROUND_CHAIN_COMPLETE",
        "completed_tasks": completed_tasks,
        "next_task": None,
        "runtime_root": str(runtime),
        "source_root": str(source),
        ...
        "credential_authority": "TV/TVC",
        "repository_writeback_performed": False,
        "public_dashboard_published": False,
        "authority_effect": "NONE_ORCHESTRATION_ONLY",
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Execute the canonical SV-DN-1 first-round task chain on one sovereign resident opportunity."
    )
    ...
    try:
        ...
        result = {
            "schema": "stegverse.sv-dn1.sovereign-execution-chain/v1",
            "state": "BLOCKED",
            "transition_id": "SV_DN1_SOVEREIGN_CHAIN_BLOCKED",
            "error": str(exc),
            "github_token_required": False,
            "credential_authority": "TV/TVC",
            "authority_effect": "NONE_ORCHESTRATION_ONLY",
