...
def build_residual_recording(*, manifest: dict[str, Any], runner_result: dict[str, Any], runner_expiry: dict[str, Any]) -> dict[str, Any]:
    recording_levels = manifest.get("recording_levels")
    if not isinstance(recording_levels, list):
        raise ValueError("manifest recording_levels must be a list")
    return {
        ...
        "recording_levels_required": recording_levels,
        "recording_levels_carried": recording_levels,
        "runner_expired": True,
...
def build_entropy_recovery(*, manifest: dict[str, Any], runner_expiry: dict[str, Any], residual_recording: dict[str, Any], custody_request: dict[str, Any], custody_record: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "reusable_task_id": manifest["reusable_task_id"],
        "manifest_hash": manifest["manifest_hash"],
        "runner_expired": runner_expiry.get("runner_expired") is True,
        "required_recording_levels_complete": residual_recording.get("recording_levels_carried") == residual_recording.get("recording_levels_required"),
        "master_records_custody_accepted": custody_record.get("destination_custody_accepted") is True,
        "master_records_reconstruction_confirmed": custody_record.get("reconstruction_confirmed") is True,
