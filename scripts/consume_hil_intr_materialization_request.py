...
The materialization request is transport intent only. It does not grant a claim,
fence, credential, heartbeat, review, publication, custody, or execution
authority. This consumer may invoke only the already-admitted HIL targeted
execution bridge.

A successfully attempted materialization is never blindly re-executed. A failed
attempt remains nonterminal and may be retried only on a later explicit
reconciliation opportunity, where the existing WorkerCoordinator claim/fence
...
def validate_request(request: dict[str, Any]) -> None:
    ...
    path = request.get("boundary_path")
    if path != ["DEVICE_SYSTEM", "STEGOS_ECOSYSTEM"]:
    ...
    *,
    ...
    receipt = {
        ...
        "state": (
            ...
            else "MATERIALIZATION_EXECUTION_BLOCKED"
        ),
        "materialization_id": request["materialization_id"],
        "request_hash": request["request_hash"],
        ...
        "successful_attempt_is_not_blindly_retried": True,
        "blocked_attempt_remains_nonterminal": completed.returncode != 0,
        "request_grants_authority": False,
...
def consume_all(
    ...
    *,
    ...
    batch = {
        ...
        "state": (
            "NO_HIL_MATERIALIZATION_REQUEST"
            if not results
            else ("BLOCKED" if blocked else "PROCESSED")
        ),
