...
REQUIRED = (
    ...
    "navigator.userAgent.includes('iPhone')",'window.isSecureContext !== true',"crypto.subtle.digest('SHA-256'",'localStorage.setItem(STORAGE_KEY, serialized)',
...
def main() -> int:
    ...
    if failures:
        [print(f'IPHONE_HB30_INLINE_CAPSULE_FAIL:{x}') for x in failures]; return 1
    print('IPHONE_HB30_INLINE_CAPSULE_PASS surface=CURRENT_USER_IPHONE transport=STEGVERSE_INLINE_SAFARI_CAPSULE authority_effect=NONE credential_authority=TV/TVC github_token_runtime_authority=NONE hosted_publication_required=false')
    return 0
