...
def validate_vector(profile,v):
    ...
    d=list(map(int,v))
    if profile=="task.v1":
        return d[1] in (0,1,2) and d[8] in (0,1,2) and d[9] in (0,1,2) and d[11] in (0,1,2) and d[12] in (0,1,2) and d[13] in (0,1,2)
    if profile=="aggregate.v1": return d[13] in (0,1,2)
...
def aggregate(children,lifecycle_state="MACHINE_OWNED"):
    ...
    totals={k:0 for k in ["critical_blockers","conflicting_claims","unassigned_work","stale_claims"]}
    threads=[]
    for c in children:
        w=int(c.get("weight",1));
        if not 1<=w<=9: raise ValueError("weight must be 1..9")
        totalw+=w
        metrics=c.get("exact_metrics",{})
        vec=c["vector"]
...
def transition(profile,a,b):
    ...
    for i,(x,y) in enumerate(zip(map(int,a),map(int,b))):
        ...
        if x==2 and y in (0,1): out.append(3); continue
        if y==2 and x in (0,1): out.append(4); continue
...
def validate_record(r):
    ...
    if any(k not in r for k in req): return False
    if not r["evidence_refs"]: return False
    return validate_vector(r["profile"],r["vector"])
def self_test():
    ...
    assert len(a)==14 and a.startswith("598753598")
    assert validate_vector("task.v1",v)
    assert len(transition("task.v1",v,v))==14
    print("COSV_SELF_TEST_PASS")
def main():
    ap=argparse.ArgumentParser(); sp=ap.add_subparsers(dest="cmd",required=True)
    sp.add_parser("self-test")
    p=sp.add_parser("validate"); p.add_argument("file")
    args=ap.parse_args()
