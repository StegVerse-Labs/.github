...
def bootstrap(
    ...
    *,
    ...
    installer = source_root / "scripts" / "install_sovereign_heartbeat_service.py"
    verifier = source_root / "scripts" / "verify_sovereign_runtime_activation.py"
    ...
    install = runner(install_cmd, check=False, capture_output=True, text=True, timeout=180, env=child_env)
    ...
    verify_cmd = [sys.executable, str(verifier), "--runtime-root", str(runtime_root)]
    verify = runner(verify_cmd, check=False, capture_output=True, text=True, timeout=180, env=child_env)
