...
KV_RETURN_OWNER="StegVerse-Labs/continuity-vault-kit"
MIR_SDK_RETURN_OWNER="StegVerse-org/StegVerse-SDK"
POST_PUBLISHER_FALSE_FLAGS=("sdk_return_binding_observed","final_stegverse_side_egress_transition_observed","interlock_intr_egress_observed","far_side_transition_observed","authentic_external_mir_endpoint_substitution_observed","communication_complete")
...
def source_root(env_name:str,repo_name:str,required:str)->Path|None:
    ...
    return None
def select_return_owner(result:dict[str,Any])->str:
    ...
    if binding.get("publisher_transition_observed") is not True:
        raise PublisherInTrConsumerError("publisher_return_transition_not_observed")
    if binding.get("authority_effect")!="NONE":
        raise PublisherInTrConsumerError("publisher_return_authority_effect_invalid")
    for field in POST_PUBLISHER_FALSE_FLAGS:
        if binding.get(field) is not False:
            raise PublisherInTrConsumerError("publisher_return_downstream_predicate_promoted:"+field)
    return MIR_SDK_RETURN_OWNER
...
def consume(runtime:Path,mid:str)->dict[str,Any]:
    ...
    verified_return=verify_artifact_return(return_bytes)
    return_owner=select_return_owner(verified_return)
    ...
    return_request=build_materialization_request(response.intent,payload_ref="runtime://"+str(response_path.relative_to(runtime)),downstream_owner_ref=return_owner)
